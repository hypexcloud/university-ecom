...
const EMAIL_CONFIG = {
  fromEmail:
    process.env.EMAIL_FROM_EMAIL || "noreply@university-ecom.vercel.app",
  fromName: process.env.EMAIL_FROM_NAME || "University Ecom",
  supportEmail:
    process.env.EMAIL_SUPPORT_EMAIL || "support@university-ecom.vercel.app",
...
console.log(
  `📧 Email configured for: ${EMAIL_CONFIG.fromEmail}${
    isVercelDomain ? " (Vercel domain)" : " (custom domain)"
  }`
);
...
export class EmailService {
  ...
  static async sendEmail(emailData: EmailData): Promise<EmailResult> {
    try {
      ...
      console.log(
        `📧 Sending email to ${emailData.to} from ${EMAIL_CONFIG.fromEmail}`
      );

      const client = getResend();
      if (!client) {
        console.warn("⚠️ Resend client unavailable, skipping email send");
        return { success: true, id: "dev-mock-" + Date.now() };
      }
  ...
  static async sendBulkEmails(emails: EmailData[]): Promise<EmailResult[]> {
    ...
    for (let i = 0; i < emails.length; i += batchSize) {
      ...
      console.log(
        `📧 Sending batch ${Math.floor(i / batchSize) + 1} of ${Math.ceil(
  ...
  private static getDefaultTemplate(
    ...
    const templates = {
      intake_confirmation: {
        subject:
          "Vielen Dank für Ihr Interesse - Ihre Antworten werden geprüft",
        html: `
      ...
      intake_approved: {
        subject:
          "Herzlichen Glückwunsch - Sie sind für unsere Kurse qualifiziert!",
        html: `
          <div style="max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif;">
            <div style="background: #000; color: #fff; padding: 40px 20px; text-align: center;">
              <h1 style="margin: 0;">🎓 University Ecom</h1>
  ...
  static async sendIntakeConfirmation(
    userEmail: string,
    firstName: string,
    userId?: string
  ) {
    const template = await this.getEmailTemplate("intake_confirmation", {
      firstName,
    });
        ...
        <div style="max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif;">
          ...
          <div style="padding: 40px 20px;">
            ...
            <div style="background: #d4edda; padding: 20px; border-left: 4px solid #28a745; margin: 20px 0;">
              ...
              <p style="margin: 5px 0 0 0; color: #155724;"><strong>Gesendet am:</strong> ${new Date().toLocaleString(
                "de-DE"
              )}</p>
              <p style="margin: 5px 0 0 0; color: #155724;"><strong>Typ:</strong> ${
                isVercelDomain ? "Vercel Domain" : "Custom Domain"
              }</p>
            </div>
          </div>
